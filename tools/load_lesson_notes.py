...
def main():
    ...
    if links:
        ...
        w("-- Form 5 lessons only; the titles are not unique across years.")
    ...
    # This file used to define student_notes() with no syllabus filter, which
    # is how Lower Sixth students ended up reading the Form 5 notes. The
    # scoped version lives in db/phase13.sql; recreating the old one here on a
    # re-run would quietly undo the fix, so this file no longer touches it.
    w("-- student_notes() is defined in db/phase13.sql, which scopes it to the")
    w("-- student's own year. Deliberately not redefined here.")
    w("DO $$")
    w("BEGIN")
    w("  IF to_regprocedure('public.student_notes(uuid)') IS NULL THEN")
    w("    RAISE NOTICE 'Run db/phase13.sql so students see only their own year.';")
    w("  END IF;")
    w("END $$;")
    w("")
    w("COMMIT;")
