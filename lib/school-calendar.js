...
/** Contact time per stream per week, for the header and the workload check. */
export function weeklyLoad(formLevel) {
  return timetableFor(formLevel).map((s) => ({
    ...
    // Periods, not minutes, is what the Ministry states a workload in. The two
    // do not divide into each other: period 9 is thirty minutes and the rest
    // are fifty, so three periods is 130 minutes on one day and 150 on another.
    periods: s.slots.reduce((t, x) => t + (periodSpan(x.start, x.end).length || 1), 0),
